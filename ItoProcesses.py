...
class Minibatch(ABC):
    ...
    def path(self,M):    
        ...
        for n in range(self.N):
            ...
            S[:,n+1,:] = self.step(t,S[:,n,:],(t1-t0),(X1-X0)) 
...
class gBmTriplet(gBm):
    ...
    def full_path(self,M):
        ...
        for n in range(self.N):
            ...
            S[:,n+1,:] = self.step(t,S[:,n,:],(t1-t0),dX) 
            W[:,n+1,:],rm[:,n+1,:],rM[:,n+1,:] = bm_triplet(np.log(S[:,n,:]),d=(self.mu-((self.sigma**2)/2)),v=self.sigma,X=dX,T=self.dt)
